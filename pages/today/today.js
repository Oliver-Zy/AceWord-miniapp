...
Page({
  ...
  */
  onLoad: async function (options) {
    ...
    Toast.loading()
    let pageInfo = await common.request({ url: `/wordcards/today` })
  ...
  */
  onPractice: async function () {
    ...
    if (prevPage.route == 'pages/mine/mine') {
      ...
      app.globalData.radioInfo.wordInfoList = await common.request({ url: `/wordinfos/search?wordlist=${wordList.join(',')}` })
      wx.navigateTo({ url: '../radio/radio' })
  ...
  */
  ...
  _onSelectActionSheetPractice: async function (e) {
    ...
    wx.navigateTo({ url: '../practice/practice?entryPage=index' })
